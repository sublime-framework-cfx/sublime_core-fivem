...
local function Checker(url, webhook, timer, isBeta, perso)
    ...
    url = from == 'tebex' and ("https://raw.githubusercontent.com/SUP2Ak/version-tebex-script/main/%s.json"):format(sl.env) or from == 'github' and ('https://api.github.com/repos/SUP2Ak/%s/releases/latest'):format(sl.env) or url
            ...
            if from == 'github' or from == 'tebex' then
                ...
                for i = 1, #_version do
                    ...
                    if current ~= minimum then
                        if current < minimum then
                            print('^9---------------------------------------------------------')
                            print(message.need_update:format(sl.env, version, lastVersion, from == 'github' and resp.html_url or from == 'tebex' and resp.link))
                            print('^9---------------------------------------------------------')
                            return
                        else break end
                    end
